...
<p align="center">
  <img src="./Screenshot_WeightPredict.png" width="85%" />
</p>
...
|  이름   | 역할 |
|:--------:|-----|
| 김민한 | 🧠 **AI 모델 및 서비스 기능 공동 리드**<br>· YOLOv11 기반 수하물 객체 탐지·인식 모델 공동 연구 및 커스터마이징<br>· 학습 데이터 전처리, 어노테이션 설계 및 반복 학습을 통한 성능 개선<br>· 객체 탐지 + 객체 인식 이중 추론 구조 도입 및 프로젝트 적용<br>· 클래스별 평균 무게 기반 수하물 무게 예측 알고리즘 설계 및 구현<br>· 수하물 분류·분석, 패킹 추천, 무게 예측, 공유 기능 페이지 프론트엔드 기획·개발<br>· 위 기능에 필요한 백엔드 API 설계 및 구현 |
| 이상민 | ⚙️ **AI 모델 및 추천 로직 공동 리드**<br>· YOLOv11 모델 학습 및 데이터 전처리 공동 수행<br>· 다중 객체 분류 로직 및 분석 파이프라인 설계<br>· 패킹 추천 기능 기획 및 백엔드 API 구현<br>· 여행 정보 페이지 개발 (해외 여행 평균 예산 데이터 스크래핑 → DB 적재 → API 및 프론트 구현) |
| 이상호 | 💻 **풀스택 및 시스템 통합 리드**<br>· Flask 기반 REST API 구조 설계 및 기능 통합<br>· Vue.js 프론트엔드 초기 구조 설계 및 상태 관리 구성<br>· Redis 세션 관리, MySQL DB 설계 및 쿼리 최적화<br>· 브랜치 병합 및 기능 통합 관리<br>· 커뮤니티 페이지 및 마이페이지 개발<br>· 프로젝트 문서화, 배포 환경 구성 및 전체 시스템 최종 통합 |
